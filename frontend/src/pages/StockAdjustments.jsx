...
export default function StockAdjustment({ onClose, onViewAdjustments }) {
  ...
  const loadStockItems = async () => {
    try {
      ...
      setStockItems(
        (itemsResult || []).filter((item) => Number(item.is_active) === 1),
      );

      setStockReport(reportResult || []);
    } catch (err) {
      setError(err?.message || "Unable to load stock items.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStockItem = (stockItemId) => {
    return stockItems.find((item) => Number(item.id) === Number(stockItemId));
  };
  ...
  const getBalance = (stockItemId) => {
    ...
    return reportItem ? Number(reportItem.balance_qty) : 0;
  };
  ...
  const validate = () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (selectedIds.has(Number(item.stock_item_id))) {
        return "The same stock item cannot be added more than once.";
      }

      selectedIds.add(Number(item.stock_item_id));
      ...
      if (
        ...
        Number(item.qty) > Number(item.balance_qty)
      ) {
        const stockItem = getStockItem(item.stock_item_id);

        return `Cannot subtract ${item.qty} ${item.unit} from ${
          stockItem?.item_name || "stock item"
        }. Available balance is ${item.balance_qty} ${item.unit}.`;
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err?.message || "Unable to save stock adjustment.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          <h4 className="mb-1">Stock Adjustment</h4>

          <small className="text-muted">
            Adjust stock for excess, damage, wastage, loss, etc.
          </small>
        </div>
            ...
            <table className="table table-bordered align-middle mb-0">
              <thead className="table-light">
                <tr>
                  ...
                  <th style={{ width: "15%" }}>Final Balance</th>
              ...
              <tbody>
                {items.map((item, index) => {
                  const finalBalance = getFinalBalance(item);
                      ...
                      <td>
                        <div className="input-group">
                          <input
                            ...
                            value={item.stock_item_id ? item.balance_qty : ""}
                            readOnly
                          />
